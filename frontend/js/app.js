...
const CollectionAPI = {
  ...
  async getCollections(userId) {
    const res = await fetch(
      `${MOOD_API_BASE}/api/v1/collections?user_id=${userId}`,
      {
        method: "GET",
        headers: this.getHeaders(),
        credentials: "include",
  ...
  async addBookToCollection(
    ...
    const res = await fetch(
      `${MOOD_API_BASE}/api/v1/collections/${collectionId}/books`,
      {
        method: "POST",
        headers: this.getHeaders(),
        credentials: "include",
  ...
  async removeBookFromCollection(collectionId, bookId) {
    const res = await fetch(
      `${MOOD_API_BASE}/api/v1/collections/${collectionId}/books/${bookId}`,
      {
        method: "DELETE",
        headers: this.getHeaders(),
        credentials: "include",
...
function renderAuthNavigation(authLink, tooltip, isAuthenticated) {
  ...
  if (isAuthenticated) {
    ...
    if (tooltip)
      tooltip.innerHTML = '<i class="fa-solid fa-id-card"></i> View Profile';
    return;
  }
  ...
  if (tooltip)
    tooltip.innerHTML = '<i class="fa-solid fa-key"></i> Access account';
}
...
async function verifyStoredAuthSession() {
  ...
  authSessionPromise = (async () => {
    ...
    const shouldProbe =
      thinksLoggedIn || storedUser || getCookie("csrf_access_token");
    if (!shouldProbe) {
      return null;
    }
    ...
    try {
      ...
      console.warn(
        "Auth verification failed; using cached session state if available.",
        error,
      );
      return storedUser;
    }
...
const SafeStorage = {
  ...
  async _saveToDB(key, value) {
    ...
    showToast(
      "Local storage full! Please sync to cloud and clear cache.",
      "error",
    );
    return false;
  },
...
const MOCK_BOOKS = [
  {
    ...
    volumeInfo: {
      ...
      description:
        "A sweeping science fiction epic set on the desert planet Arrakis. Dune explores complex themes of politics, religion, and man's relationship with nature. Paul Atreides must navigate a treacherous path to becoming the mysterious Muad'Dib.",
  ...
  },
  {
    ...
    volumeInfo: {
      ...
      description:
        "Orwell's chilling prophecy of a totalitarian future where Big Brother is always watching. A profound exploration of surveillance, truth, and the resilience of the human spirit.",
  ...
  },
  {
    ...
    volumeInfo: {
      ...
      description:
        "In a hole in the ground there lived a hobbit. Join Bilbo Baggins on an unexpected journey across Middle-earth, encountering dragons, dwarves, and a rigorous test of courage.",
  ...
  },
  {
    ...
    volumeInfo: {
      ...
      authors: ["Jane Austen"],
      description:
        "A timeless romance of manners and misunderstanding. Elizabeth Bennet's wit matches Mr. Darcy's pride in this sharp social commentary that remains one of the most loved novels in English literature.",
  ...
  },
  {
    ...
    volumeInfo: {
      ...
      description:
        "The quintessential novel of the Jazz Age. Jay Gatsby's obsessive love for Daisy Buchanan drives a tragic tale of wealth, illusion, and the American Dream.",
  ...
  },
  {
    ...
    volumeInfo: {
      ...
      description:
        "A groundbreaking narrative of humanity's creation and evolution. Harari explores the ways in which biology and history have defined us and enhanced our understanding of what it means to be 'human'.",
  ...
  },
  {
    ...
    volumeInfo: {
      ...
      authors: ["Andy Weir"],
      description:
        "A lone astronaut must save the earth from disaster in this gripping tale of survival and scientific discovery. Full of humor and hard science, it is a celebration of human ingenuity.",
...
function getFallbackBooks(query, maxResults = 5) {
  ...
  const ranked = MOCK_BOOKS.map((book) => ({
    book,
    score: scoreMockBook(book, queryTerms),
...
class BookRenderer {
  ...
  async createBookElement(bookData, shelf = null) {
    ...
    const progress =
      typeof bookData.progress === "number" ? bookData.progress : 0;
...
`;
    ...
    const updateBtn = () => {
      addBtn.innerHTML = this.libraryManager.findBook(id)
    ...
    this.fetchAIVibe(title, authors, volumeInfo.description || "").then(
      (aiVibe) => {
        if (aiVibe) {
          // Strip any accidental prefix the AI might return
          const cleanVibe = aiVibe.replace(
            /^(Bookseller's Note:|Note:|Recommendation:)\s*/i,
            "",
          );
  ...
  generateVibe(text, categories = []) {
    ...
    if (lowerCats.includes("classic") || lowerText.includes("classic"))
      return "A timeless tale that defined a genre.";
    if (lowerCats.includes("romance") || lowerText.includes("love"))
      return "A heartwarming story of connection.";
    if (
      lowerCats.includes("mystery") ||
      lowerText.includes("murder") ||
      lowerText.includes("detective")
    )
      return "Full of twists that keep you guessing.";
    if (lowerCats.includes("fantasy") || lowerText.includes("magic"))
      return "A magical escape to another world.";
    if (lowerCats.includes("fiction") || lowerText.includes("novel"))
      return "A compelling narrative voice.";
    if (lowerCats.includes("history") || lowerText.includes("war"))
      return "A journey into the past.";
    if (lowerCats.includes("science") || lowerText.includes("space"))
      return "Opens your mind to new possibilities.";
  ...
  openModal(book) {
    ...
    const isInLibrary =
      this.libraryManager &&
      typeof this.libraryManager.findBook === "function" &&
      this.libraryManager.findBook(book.id);
    ...
    if (previewBtn) {
      previewBtn.onclick = () => {
        if (window.BookPreview && book.id) {
          window.BookPreview.open(
            book.id,
            book.volumeInfo.title || "Book Preview",
          );
        }
      };
    }
    ...
    if (purchaseLinksEl) {
      ...
      if (book.volumeInfo.industryIdentifiers) {
        const identifier = book.volumeInfo.industryIdentifiers.find(
      ...
      fetch(
        `${MOOD_API_BASE}/books/purchase-links?title=${title}&author=${author}&isbn=${isbn}`,
      )
        .then((res) => res.json())
        .then((data) => {
          if (data.success && data.links && data.links.length > 0) {
            ...
            purchaseLinksEl.innerHTML =
              '<p class="modal-subtitle" style="margin: 0; font-size: 0.85rem; opacity: 0.7;">No purchase links available.</p>';
          }
        })
        .catch((err) => {
          console.error("Failed to load purchase links", err);
          purchaseLinksEl.innerHTML =
            '<p class="modal-subtitle" style="margin: 0; font-size: 0.85rem; opacity: 0.7;">Failed to load purchase links.</p>';
        });
      // Explore Mood Button
      const moodBtnModal = document.getElementById("modal-mood-btn");
      if (moodBtnModal) {
        moodBtnModal.onclick = () => {
          this.exploreBookMood(
            book.volumeInfo.title,
            book.volumeInfo.authors?.join(", ") || "",
          );
        };
      ...
      const modalBody =
        ...
        modal.querySelector(".book-details-content");
      const actions =
        modal.querySelector(".modal-actions") ||
        modal.querySelector(".book-actions-section");

      if (actions) {
        // Remove existing tagging section if re-opening
        const existing = actions.parentNode.querySelector(
          ".emotion-tagging-section",
        );
      ...
      emotionContainer.querySelectorAll(".emotion-tag").forEach((tag) => {
        tag.onclick = async () => {
          ...
          if (this.libraryManager) {
            await this.libraryManager.updateBook(book.id, {
              moods: book.moods,
            });
          }
        };
      ...
      let collectionsSection = document.getElementById(
        "modal-discovery-collections-tagging",
      );
      if (!collectionsSection) {
        ...
        collectionsSection.style.cssText =
          "margin-top: 15px; margin-bottom: 15px; padding: 1rem; background: rgba(255,255,255,0.02); border-radius: 8px; border: 1px solid rgba(255,255,255,0.05);";
      }
      ...
      if (actions) {
        ...
        const existing = modalBody.querySelector(
          "#modal-discovery-collections-tagging",
        );
        if (existing) existing.remove();
      ...
      const userObj =
        typeof parseStoredUser === "function" ? parseStoredUser() : null;
      if (!userObj) {
        ...
        (async () => {
          try {
            ...
            const listEl = document.getElementById(
              "modal-discovery-collections-list",
            );
            if (!listEl) return;
            ...
            colsWithItems.forEach((col) => {
              const existingItem = col.items.find(
                (item) => item.google_books_id === book.id,
              );
              const isChecked = !!existingItem;
              const label = document.createElement("label");
              label.style.cssText =
                "display: flex; align-items: center; gap: 8px; font-size: 0.85rem; color: var(--text-main); cursor: pointer; user-select: none; margin-bottom: 4px;";

              const checkbox = document.createElement("input");
              checkbox.type = "checkbox";
              checkbox.checked = isChecked;
              checkbox.style.cssText =
                "cursor: pointer; width: 15px; height: 15px; margin: 0;";
              ...
              checkbox.onchange = async () => {
                ...
                try {
                  if (checkbox.checked) {
                    const authorStr = Array.isArray(book.volumeInfo.authors)
                      ? book.volumeInfo.authors.join(", ")
                    ...
                    if (bookId) {
                      await window.CollectionAPI.removeBookFromCollection(
                        col.id,
                        bookId,
                      );
  ...
  async exploreBookMood(title, author) {
    ...
    const renderResults = (analysis) => {
      ...
      if (analysis.primary_moods && analysis.primary_moods.length > 0) {
        ...
        tagsContainer.innerHTML =
          '<span style="font-size: 0.9rem; color: var(--text-muted);">No distinct moods detected.</span>';
      }
      ...
      modal.querySelector("#mood-modal-sentiment-fill").style.width =
        `${percentage}%`;
      modal.querySelector("#mood-modal-sentiment-desc").textContent =
        `${analysis.mood_description || "Sentiment analyzed successfully."} (Score: ${compoundScore.toFixed(2)})`;

      // Render vibe
      modal.querySelector("#mood-modal-vibe").innerHTML =
        `<p>${escapeHTML(analysis.bibliodrift_vibe || "A quiet read with deep undertones.")}</p>`;
      ...
      const confidenceScore = analysis.analysis_confidence
        ...
        : 50;
      modal.querySelector("#mood-modal-meta").textContent =
        `Analyzed ${totalReviews} Goodreads reviews. Vibe confidence: ${confidenceScore}%.`;
    };
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (analysis && analysis.success) {
          ...
          showError(
            analysis?.error || "Could not parse mood analysis for this book.",
          );
        }
      } else {
        if (res.status === 429) {
          ...
          showError(
            "Mood analysis is currently offline (missing backend dependencies).",
          );
        } else if (res.status === 404) {
          showError("No Goodreads reviews found for this title to analyze.");
        } else {
          showError(
            `Failed to fetch mood analysis (Server error: ${res.status}).`,
          );
        }
      }
    } catch (err) {
  ...
  async renderCuratedSection(query, elementId, maxResults = 5) {
    ...
    try {
      ...
      const data = client
        ? await client.fetchVolumes(query, {
            maxResults,
        ...
        : await (async () => {
            ...
            const res = await fetch(
              `${API_BASE}?q=${encodedQuery}&maxResults=${maxResults}&printType=books${keyParam}`,
            );
            if (!res.ok) {
  ...
  async renderMoodCategorySection(categoryConfig, elementId, maxResults = 5) {
    ...
    try {
      ...
      if (categoryBooks.length === 0) {
        throw new Error(
          `No books returned for category: ${categoryConfig.category}`,
        );
      }

      const resolvedBooks = await this.resolveCategoryBooks(categoryBooks);
      if (resolvedBooks.length > 0) {
        await this.renderBookCards(
          container,
          resolvedBooks.slice(0, maxResults),
        );
        return;
      }

      throw new Error(
        `Could not resolve Google Books matches for category: ${categoryConfig.category}`,
      );
    } catch (err) {
      console.error(
        `Failed to load category shelf "${categoryConfig.category}"`,
        err,
      );
      await this.renderCuratedSection(
        categoryConfig.fallbackQuery,
        elementId,
        maxResults,
      );
    }
  ...
  async resolveCategoryBooks(categoryBooks) {
    ...
    for (const item of categoryBooks) {
      ...
      try {
        const client = window.GoogleBooksClient;
        const data = client
          ? await client.fetchVolumes(searchQuery, {
              maxResults: 1,
              extraParams: "&printType=books",
            })
          : await (async () => {
              const keyParam = GOOGLE_API_KEY ? `&key=${GOOGLE_API_KEY}` : "";
              const res = await fetch(
                `${API_BASE}?q=${encodeURIComponent(searchQuery)}&maxResults=1&printType=books${keyParam}`,
              );
              if (!res.ok) {
  ...
  async renderBookCards(container, books) {
    ...
    if (!books || books.length === 0) {
      container.innerHTML =
        '<p class="empty-state">No books available for this collection.</p>';
      return;
    }
    ...
    if (container.children.length === 0) {
      container.innerHTML =
        '<p class="empty-state">Failed to load books. Please check your connection.</p>';
    }
  }
}
...
class SearchFilterManager {
  constructor(container, books, renderer) {
    ...
    this.activeFilter =
      urlParams.get("mood") || sessionStorage.getItem("active_mood_filter");
          ...
          if (backFace) {
            ...
            if (!tagsEl) {
              ...
              tagsEl.style.cssText =
                "margin-bottom: 0.8rem; display: flex; flex-wrap: wrap; gap: 4px;";
              backFace.appendChild(tagsEl);
            }
            tagsEl.innerHTML = moods
              .map(
                (m) => `
  ...
  renderFilterChips() {
    ...
    sortedMoods.forEach((mood) => {
      const isChipActive =
        this.activeFilter &&
        this.activeFilter.toLowerCase() === mood.toLowerCase();
  ...
  applyFilter() {
    ...
    const existingEmptyState = this.container.querySelector(
      ".empty-filter-state",
    );
    ...
    if (visibleCount === 0 && this.books.length > 0) {
      ...
      const activeMoodName =
        this.activeFilter.charAt(0).toUpperCase() + this.activeFilter.slice(1);
...
class LibraryManager {
  ...
  async init() {
    ...
    if (stored) {
      try {
        ...
        console.error(
          "[Library] Failed to parse stored library, resetting to empty.",
          e,
        );
      }
    }
  ...
  async _queueMutation(action, book, extra = {}) {
    ...
    const existingMutations = (
      await window.db.syncQueue.where("userId").equals(user.id).toArray()
    ).filter((mutation) => mutation.bookId === snapshot.id);

    if (action === "remove") {
      await Promise.all(
        existingMutations.map((mutation) =>
          window.db.syncQueue.delete(mutation.id),
        ),
      );
      await this.updateSyncStatus();
      return;
    ...
    const pendingAdd = existingMutations.find(
      (mutation) => mutation.action === "add",
    );
    if (pendingAdd && (action === "move" || action === "update")) {
      await window.db.syncQueue.put({
        ...
        shelf:
          action === "move"
            ? extra.toShelf || pendingAdd.shelf
            : pendingAdd.shelf,
        payload: {
          ...(pendingAdd.payload || {}),
          ...extra,
  ...
  async _applyQueuedMutation(mutation) {
    ...
    if (mutation.action === "add") {
      ...
      const payload = {
        ...
        authors: localBook.volumeInfo?.authors
          ? localBook.volumeInfo.authors.join(", ")
          : "",
  ...
  async flushPendingLibraryMutations() {
    ...
    const pendingMutations = await window.db.syncQueue
      .where("userId")
      .equals(user.id)
      .sortBy("createdAt");
          ...
          if (existing) {
            ...
            if (item.version > (localBook.version || 0)) {
              if (existing.shelf !== item.shelf_type) {
                ...
                this.library[existing.shelf] = this.library[
                  existing.shelf
                ].filter((b) => b.id !== item.google_books_id);
                // Add to new shelf
  ...
  async syncLocalToBackend(user) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data.conflicts > 0) {
          showToast(
            `Synced ${data.message} (${data.conflicts} conflicts resolved by server)`,
            "info",
          );
        } else {
  ...
  sortLibrary(criteria) {
    const sortFn = (a, b) => {
      switch (criteria) {
        ...
        case "title_desc":
          return (b.volumeInfo.title || "").localeCompare(
            a.volumeInfo.title || "",
          );
        case "author":
        case "author_asc":
          const authorA =
            (a.volumeInfo.authors && a.volumeInfo.authors[0]) || "";
          const authorB =
    ...
    ["current", "want", "finished"].forEach((shelf) => {
      if (this.library[shelf]) {
        ...
        if (
          window.bookshelf3D &&
          typeof window.bookshelf3D.refreshShelves === "function"
        ) {
  ...
  async addBook(book, shelf) {
    ...
    if (typeof window.logReadingActivity === "function") {
      window.logReadingActivity(
        "add",
        `Added "${book.volumeInfo.title}" to ${shelf}`,
      );
    }
  ...
  async updateBook(id, updates) {
    ...
    if (updates.progress === 100 && shelf !== "finished") {
      ...
      if (typeof window.logReadingActivity === "function") {
        window.logReadingActivity(
          "finish",
          `Finished reading "${book.volumeInfo.title}"`,
        );
      }
    }
  ...
  async moveBook(id, toShelf) {
    ...
    this.library[fromShelf] = this.library[fromShelf].filter(
      (b) => b.id !== id,
    );
    ...
    if (toShelf === "finished" && book.progress !== 100) {
      ...
      toShelf === "current" &&
      (book.progress == null || book.progress === 100)
    ) {
      book.progress = 0;
    }
  ...
  async renderShelf(shelfName, elementId) {
    ...
    if (books.length === 0) {
      ...
      container.innerHTML =
        '<div class="empty-state">This shelf is empty.</div>';
      return;
    }
...
class ThemeManager {
  ...
  applyTheme(theme) {
    ...
    if (this.toggleBtn) {
      ...
      if (icon) {
        icon.className = isNight ? "fa-solid fa-sun" : "fa-solid fa-moon";
      }
      ...
      this.toggleBtn.setAttribute("aria-pressed", String(isNight));
    }
  }
}
...
class GenreManager {
  ...
  async fetchBooks(genre) {
    ...
    try {
      const client = window.GoogleBooksClient;
      const data = client
        ? await client.fetchVolumes(`subject:${genre}`, {
            maxResults: 20,
            extraParams: "&langRestrict=en&orderBy=relevance",
          })
        : await (async () => {
            const keyParam = GOOGLE_API_KEY ? `&key=${GOOGLE_API_KEY}` : "";
            const response = await fetch(
              `${API_BASE}?q=subject:${genre}&maxResults=20&langRestrict=en&orderBy=relevance${keyParam}`,
            );
            if (!response.ok) {
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  window.dispatchEvent(
    new CustomEvent("bibliodrift:library-manager-ready", {
      detail: { libraryManager: libManager },
    }),
  );
  libManager.ready().then(() => {
    window.dispatchEvent(
      new CustomEvent("bibliodrift:library-manager-synced", {
        detail: { libraryManager: libManager },
      }),
    );
  });
  ...
  if (query && document.getElementById("search-results-section")) {
    ...
    document
      .querySelectorAll(".curated-section:not(#search-results-section), .hero")
      .forEach((el) => {
    ...
    const discoveryShelves = [
      ...
      {
        type: "query",
        query: "subject:thriller suspense",
        elementId: "row-thriller",
      },
    ];
    (async () => {
      try {
  ...
  if (document.getElementById("profile-page")) {
    ...
    if (Object.keys(categoryCounts).length > 0) {
      topVibe = Object.keys(categoryCounts).reduce((a, b) =>
    ...
    const loadExtendedStats = async () => {
      ...
      try {
        ...
        const statsResponse = await fetch(
          `${MOOD_API_BASE}/stats?user_id=${user.id}&year=${currentYear}`,
          {
        ...
        if (statsResponse.ok) {
          ...
          if (stats.goal) {
            ...
            if (progressText)
              progressText.textContent = `${completed} / ${target} books`;
            if (barGoal && target > 0) {
            ...
            const progressText = document.getElementById("goal-progress-text");
            if (progressText)
              progressText.textContent = "No goal set for this year";
          }
        }

        // Fetch Leaderboard
        const lbResponse = await fetch(
          `${MOOD_API_BASE}/stats/leaderboard?year=${currentYear}&limit=5`,
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        );
    ...
    if (editGoalBtn) {
      editGoalBtn.addEventListener("click", () => {
        goalEditGroup.style.display =
          goalEditGroup.style.display === "none" ? "flex" : "none";
    ...
    const renderErrorIdentityState = () => {
      const identityContent = document.getElementById(
        "reader-identity-content",
      );
    ...
    const fetchAndRenderArchetype = async (genres, reviewsList) => {
      ...
      if (!response.ok) {
        throw new Error(
          `Failed to fetch archetype (Status: ${response.status})`,
        );
      }
      ...
      const sentimentScore =
        typeof profile.sentiment_score === "number"
          ? profile.sentiment_score.toFixed(2)
          : "0.00";
      const cluster =
        typeof profile.reader_cluster === "number"
          ? profile.reader_cluster === -1
            ? "Unclassified"
            : `Group #${profile.reader_cluster}`
          : "Unclassified";

      const archetypeDescriptions = {
        "Deep Thinker":
          "Drawn to philosophy, existential questions, and reflective/psychological themes.",
        "Emotional Reader":
          "Connects deeply with romance, relationships, emotional journeys, and human feelings.",
        "Dark Reader":
          "Enjoys crime, violence, psychological thrillers, horror, and mystery.",
        "Adventurous Reader":
          "Seeks sci-fi, fantasy, action-packed adventures, and exploration.",
      };
      const desc =
        archetypeDescriptions[archetype] ||
        "Based on your current reading habits and preferences.";
      ...
      if (submitBtn) {
        submitBtn.addEventListener("click", async () => {
          const textInput = document
            .getElementById("onboarding-review-text")
            .value.trim();
          if (!textInput) return;
    ...
    const loadReaderIdentity = async () => {
      ...
      try {
        ...
        const reviewsResponse = await fetch(
          `${MOOD_API_BASE}/users/${user.id}/reviews`,
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        );
    ...
    if (totalBooks > 0) {
      setTimeout(() => {
        if (barFinished)
          barFinished.style.width = `${(finishedCount / totalBooks) * 100}%`;
        if (barCurrent)
          barCurrent.style.width = `${(currentCount / totalBooks) * 100}%`;
    ...
    if (progressGrid) {
      ...
      if (currentBooks.length === 0) {
        progressGrid.innerHTML =
          '<div class="empty-state"><p>No books currently in progress. <a href="library.html">Visit your library</a> to start reading.</p></div>';
      } else {
        progressGrid.innerHTML = "";
        currentBooks.forEach((book) => {
          const title = book.volumeInfo?.title || book.title || "Untitled";
          const author =
            book.volumeInfo?.authors?.[0] || book.author || "Unknown Author";
          const cover =
            book.volumeInfo?.imageLinks?.thumbnail || book.cover || "";
          const progress =
    ...
    document
      ...
      .addEventListener("click", async () => {
        try {
          ...
          await fetch(`${MOOD_API_BASE}/logout`, {
            method: "POST",
            credentials: "include",
          });
        } catch (e) {
...
async function handleAuth(event) {
  ...
  try {
    ...
    if (csrfToken) {
      ...
      console.warn(
        "[Security] No CSRF token found in DOM. Request may be rejected by server.",
      );
    }

    const res = await fetch(
      `${MOOD_API_BASE}${endpoint.replace("/api/v1", "")}`,
      fetchOptions,
    );
    ...
    if (res.ok) {
      ...
      if (typeof showToast === "function")
        showToast(
          `${mode === "login" ? "Welcome back" : "Welcome"}, ${data.user.username}!`,
          "success",
        );
      ...
      if (window.libManager) {
        if (typeof showToast === "function")
          showToast("Syncing your library...", "info");
...
const KeyboardShortcuts = {
  ...
  shortcuts: {
    ...
    r: {
      action: "markCurrentlyReading",
      description: "Mark as Currently Reading",
    },
    ...
    switch (action) {
      ...
      case "focusSearch":
        ...
        const searchInput = document.querySelector(
          'input[type="search"], input.search, [placeholder*="search" i]',
        );
  ...
  displayHelpMenu() {
    ...
    alert(
      "BiblioDrift Keyboard Shortcuts\n\n" +
  ...
  setBookFocus(bookElement) {
    ...
    if (bookElement) {
      ...
      if (window.bookshelfRenderer && bookId) {
        const storage = window.libManager?.getLibrarySnapshot?.() || {
          current: [],
          want: [],
          finished: [],
        };
  ...
  moveCurrentBookToShelf(targetShelf) {
    ...
    // Move the book
    window.bookshelfRenderer.moveBook(
      currentBook.id,
      currentShelf,
      targetShelf,
    );
...
if ("serviceWorker" in navigator) {
  window.addEventListener("load", () => {
    navigator.serviceWorker
      .register("/sw.js")
      .then((reg) =>
        console.log("BiblioDrift Service Worker registered successfully!", reg),
      )
      .catch((err) =>
...
function handleConnectivityChange() {
  ...
  if (!navigator.onLine) {
    console.warn(
      "🌐 Connection dropped. Switching to local sanctuary archives...",
    );
    ...
    console.log(
      "🌐 Connection restored! Connected back to the live backend cloud server.",
    );
    if (offlineIndicator) {
...
class ReadingMoodQuizManager {
  ...
  init() {
    this.section =
      document.getElementById("reading-mood-quiz") ||
      document.getElementById("readingMoodQuiz");
    ...
    if (
      !this.quizOptionGroups.length ||
      !this.generateBtn ||
      !this.results ||
      !this.moodTagsContainer ||
      !this.retakeBtn
    ) {
      return;
    }
  ...
  _wireButtons() {
    this.generateBtn.addEventListener("click", async () => {
      ...
      try {
        SafeStorage.set("readingMoodTags", JSON.stringify(tags));
      } catch (e) {
        localStorage.setItem("readingMoodTags", JSON.stringify(tags));
      }
      ...
      try {
        if (
          this.renderer &&
          typeof this.renderer.renderCuratedSection === "function"
        ) {
          this.renderer.renderCuratedSection(
            generatedMoodQuery,
            "mood-quiz-results-grid",
            16,
          );
        }
      } catch (e) {
    ...
    this.retakeBtn.addEventListener("click", () => {
      ...
      try {
        SafeStorage.remove("readingMoodTags");
      } catch (e) {
        localStorage.removeItem("readingMoodTags");
      }
      this._updateGenerateButtonState();
    });
  }
  ...
  loadSavedMoodTags() {
    let saved = null;
    try {
      saved = SafeStorage.get("readingMoodTags");
    } catch (e) {
      saved = localStorage.getItem("readingMoodTags");
    }
    if (saved) {
      try {
...
function _startReadingMoodQuiz() {
  const startIfReady = () => {
    ...
    if (window.libManager && window.renderer) {
      window.moodQuizManager = new ReadingMoodQuizManager(
        window.libManager,
        window.renderer,
      );
      ...
      window.addEventListener(
        "bibliodrift:library-manager-ready",
        () => {
          if (window.libManager && window.renderer) {
            window.moodQuizManager = new ReadingMoodQuizManager(
              window.libManager,
